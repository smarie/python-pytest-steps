...
pkg_name = "pytest_steps"
gh_org = "smarie"
gh_repo = "python-pytest-steps"
...
def tests(session: PowerSession, coverage, pkg_specs):
    ...
    if not coverage:
        ...
        # First RAW
        session.run2("coverage run --source {pkg_name} -m pytest -v {pkg_name}/tests_raw/"
                     "".format(pkg_name=pkg_name, test_xml=Folders.test_xml, test_html=Folders.test_html))

        # Then META (append and generate report)
